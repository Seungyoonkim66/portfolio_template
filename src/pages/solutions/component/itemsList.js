            ...
            <FullScreen bg={thumbnail}  size={100} padding='10rem 0 0 5rem'>
                <Container className='container' maxWidth='lg'>
                    ...
                    <div className='introText'>
                        <Link to={`/items${link}`}><div className='title'>{title}</div></Link>
                        <ItemNavList items={items} link={link} />
                        <Link to='/items'><div className='backBtn'><ArrowBackSharpIcon className='icon' />&ensp;다른 솔루션 더보기</div></Link>
                    </div>

                    <div className='contentsBox'>
                        <Switch>
                            <Route path={`/items${link}`} exact render={() => <SolutionIntro desc={desc} descKo={descKo} />} />
                            {items.map((item, i) => <Route key={i} path={`/items${link}${item.link}`} render={() => <ItemDetail item={item} />} />)}
                        </Switch>
                    </div>
                </Container>
            </FullScreen>
